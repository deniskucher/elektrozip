$.widget('xb.abstractManager', {
    ...
    _create: function () {
        ...
        */
        ...
        widget.element.on('enter', '#'+widget.options.tableName+'>tbody>tr>td[contenteditable]', function(_e) {
            ...
            widget._setValue(widget.options.moduleName.toString().toLowerCase()+'.set'+widget.options.entityName.toString().toLowerCase()+fieldName.toString().toLowerCase(), $(this).closest('tr').data('id'), fieldName, $(this).text(), null);
        });
        ...
        $('body').on('click', '#'+widget.options.tableName+'>tfoot>tr>td>.btn-add', function() {
            if (widget.options.createInputs.length)
                widget._openInsertRecordModal($(this).closest('tr').data('id'), 'end');
            else {
        ...
        widget.element.on('focus', '#selectedRecordOptions form>table>tbody>tr>td.optionstr', function (_e) {
            $(this).selectText();
            $(this).data('value', $(this).text());
            $(this).closest('tr').css('background-color', 'lightgreen');
        });
    ...
    _refresh: function () {
        ...
        widget.element.empty().append(
            searchLine.css({'margin': '10px 0px', 'display': widget.options.globalSearch ? 'block' : 'none'}).append(
                $('<form/>', {id: 'search'+widget.options.tableName, class: 'form-inline'}).css({display: 'inline-block'}).append(
                    $('<div/>', {class: 'form-group'}).append(
                        $('<input/>', {class: 'form-control', type: 'text', name: 'query'}).css({'margin':'0 5px', 'display': widget.options.globalSearch ? 'inline' : 'none'}).attr('placeholder','Search...')
                    ),
                    $('<button/>', {class: 'btn btn-primary'}).css('margin','0 5px').text(widget.options.button)
                )
            )
        );
        ...
        if(footer.length) {
            widget.element.find('table#'+widget.options.tableName+'>tfoot').prepend($('<tr/>', {class: 'footer'}));
            ...
            for(var i=0; i<columnsCount; i++)
            {
                ...
                for(var y in footer)
                {
                    ...
                    if(footerItem.colNumber == i)
                    {
                        ...
                        if(footerItem.function == 'sum') $footerTd.append($('<span/>', {class: 'sum'+footerItem.key}).html('0'));
     ...
     * @updated 2017.05.19 added "_targetId" to set target ID from options if it need
     *
     * INFO:
    ...
    _appendRecord: function (_id, _data, _position, _baseId) {
        var widget = this;
        var $tr = $('<tr/>', {'data-id': _data.id}).append($('<td/>').css('text-align','right').text(_data.id));
    ...
    _renderRecord: function (_id, _data, $tr) {
        ...
        for (var i = 0, len = fields.length; i < len; ++i)
        {
            ...
            if(!field.type || field.type == 'text')
            {
                ...
                for(var y in footer)
                {
                    ...
                    if(footerItem.key === field.key)
                    {
                        if(footerItem.function == 'sum')
                        {
                            ...
                            widget.element.find('table#'+widget.options.tableName+'>tfoot>tr.footer>td:eq('+footerItem.colNumber+')>.sum'+footerItem.key).html(parseInt(currentFooterItemVal)+parseInt(value));
                        }
                    }
                }
                ...
                $tr.append($('<td/>', {'contenteditable': field.editable, 'data-key': field.key, 'modallang': field.modalLang, 'title': title}).css('text-align', field.textAlign ? field.textAlign : 'left').text(value));
            }
    ...
    _showRecordBlock: function (_id){
        ...
        if(widget.options.view.enable)
        {
            ...
            {
                $recordBlock = $('<div/>', {class: 'col-md-6 col-sm-6 col-xs-12', id: 'selectedRecord'}).css({margin: '0 auto', maxWidth: '400px', height: '150px'});
    ...
    _renderSelectedRecord: function(_data){
        ...
        if(widget.options.view.enable)
        {
            ...
            if ($recordBlock.length == 0)
            {
                ...
                $recordForm = $('<form/>').appendTo($recordBlock);
                $('body').find('#content').append($recordBlock);
            }
            else
            {
            ...
            for(var fieldId in fields)
            {
                ...
                switch(field.inputType){
                    ...
                    //     {
                    //         $label = $('<label/>', {'for': field.key}).text(field.name);
                    //         $input = $('<select/>', {'name': field.key,'class': 'form-control'});
                    //         $input.append($('<option/>', {'value': 0}).attr('selected','selected').text('Select '+field.name));
                    //         $group.append($label, $input);
                    //         widget._appendSelectWithOptions($input, field);
                    //     }
                    //     break;
                }
            }
    ...
    _openInsertRecordModal:function (_baseId, _position) {
        ...
        if(_position == 'before' || _position == 'after') $modal.data({'baseId': _baseId, 'position': _position});
